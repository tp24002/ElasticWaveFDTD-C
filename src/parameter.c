...
void parameter(Range *ran,Diff *dif, Pml *pml,Medium *med,Object *con,Object *clack,Coord *out,Inpaluse *ip){
    ...
    initCoord(&out[0],center.x - halfconmin, center.y, center.z);
    initCoord(&out[1],center.x + halfconmin, center.y, center.z);
    initCoord(&out[2],center.x, center.y - halfconmin, center.z);
    initCoord(&out[3],center.x, center.y + halfconmin, center.z);
